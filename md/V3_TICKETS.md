...
The real value of the embedding store is **cross-session**: on a user's second run with the same tech stack, the retrieval step (V3-13) can pull from chunks stored in their previous runs, providing richer and more targeted context without re-running Tavily.
...
With chunks stored in pgvector from V3-12, the retrieval step enhances idea generation on **subsequent runs** for the same user and tech stack. This is the key distinction from the storage step:

- **Storage (V3-12):** Happens in the background after the current run's response is already returned. Builds the knowledge store.
- **Retrieval (V3-13):** At the start of idea generation, query the user's previously stored chunks (from past runs) and use them as supplemental context for the LLM.
...
This is controlled by the `ENABLE_RAG` environment variable from V3-12. No additional flag is needed.
...
2. In the `generate_ideas` node in `graph.py`, when `ENABLE_RAG=true`, build a query string from tech stack, domain, and level from state. Call `embedder.embed_chunks` with this single string and the user's OpenAI API key to get the query vector. Call `retriever.retrieve_top_k` with the current `user_id` and query vector.
